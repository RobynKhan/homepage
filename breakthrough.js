...
(function () {
    ...
    const BRICK_COLORS = ['#ff2020', '#ff8800', '#ffe600', '#00e500', '#00d4ff', '#c800ff'];
    ...
    function updateHUD() {
        ...
        if (hudTotal) hudTotal.textContent = String(apiData.me.total_score  || 0).padStart(5, '0');
        if (hudTop) {
            const ts = String(apiData.top.best_run_score || 0).padStart(5, '0');
            hudTop.textContent = 'TOP: ' + ts + ' (' + (apiData.top.username || '---') + ')';
        }
    }
    ...
    function update() {
        ...
        if (
            ...
            state.ball.y + BALL_R <= paddleY + PADDLE_H + Math.abs(state.ball.dy) + 1 &&
        ...
        if (state.bricks.every(function (b) { return !b.alive; })) {
            state.score += 100;
            state.won = true;
            updateHUD();
    ...
    async function loadAPIState() {
        try {
            const res = await fetch(API + '?action=state', { credentials: 'same-origin' });
            if (res.ok) {
    ...
    function initBreakout() {
        ...
        if (panel) {
            const mo = new MutationObserver(function () {
                ...
                const isVisible = panel.classList.contains('sheet-open') ||
                    (panel.style.display && panel.style.display !== 'none');
