...
function addToQueue(trackUri, trackName, artistName, albumArt) {
  queue.push({
    uri: trackUri,
    name: trackName,
    artist: artistName,
...
function updateQueueUI() {
  ...
  queue.forEach((track, index) => {
    ...
    li.innerHTML = `
      ${track.art ? `<img src="${track.art}" alt="" style="width:36px;height:36px;border-radius:6px;object-fit:cover;flex-shrink:0">` : ""}
      <div class="track-meta" style="flex:1;overflow:hidden">
        <span class="track-title">${track.name}</span>
        <span class="track-artist">${track.artist}</span>
      </div>
      <button class="remove-btn" onclick="removeFromQueue(${index})" title="Remove">&times;</button>
    `;
...
async function loadPlaylists() {
  ...
  data.items.forEach((playlist) => {
    ...
    li.onclick = () => loadTracks(playlist.id, playlist.name);
    list.appendChild(li);
  });
}
...
async function loadTracks(playlistId, playlistName) {
  document
    .querySelectorAll("#playlist-list li")
    .forEach((l) => l.classList.remove("active"));
  const active = document.querySelector(`[data-id="${playlistId}"]`);
...
function renderTracks(tracks) {
  ...
  tracks.forEach((track) => {
    ...
    li.innerHTML = `
      ...
      <div class="track-actions" style="display:flex;gap:6px;flex-shrink:0">
    ...
    queueBtn.addEventListener("click", () =>
      addToQueue(track.uri, track.name, track.artists[0].name, art),
    );
...
window.onSpotifyWebPlaybackSDKReady = async () => {
  ...
  spotifyToken = token; // save for later API calls
  ...
  // Load playlists into sidebar once connected
  loadPlaylists();
...
async function loadPlaylists() {
  ...
  data.items.forEach((pl) => {
    ...
    li.dataset.id = pl.id;
    li.title = pl.name;
    li.addEventListener("click", () => loadPlaylistTracks(pl.id, pl.name, li));
    list.appendChild(li);
  });
}

async function loadPlaylistTracks(playlistId, playlistName, liEl) {
  // Mark active
  document
    .querySelectorAll("#playlist-list li")
    .forEach((l) => l.classList.remove("active"));
  ...
  const data = await res.json();
  renderTrackList(data.items.map((item) => item.track).filter(Boolean));
}
